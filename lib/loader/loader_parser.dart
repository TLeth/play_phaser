...
class LoaderParser {
  ...
  static bitmapFont(Game game, Document xml, String cacheKey, int xSpacing, int ySpacing) {
    ...
    for (int i = 0; i < letters.length; i++) {
      ...
      PIXI.Rectangle textureRect = new PIXI.Rectangle(int.parse(letters[i].getAttribute('x')), int.parse(letters[i].getAttribute('y')), int.parse(letters[i].getAttribute('width')), int.parse(letters[i].getAttribute('height')));
